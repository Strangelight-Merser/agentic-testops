...
- 通过 `--llm-explain` 启用可选的 LLM 分析层：把结构化失败证据发送给 Anthropic API，在确定性诊断旁渲染顾问性的根因解释。需要 `ANTHROPIC_API_KEY`；缺少时审计照常运行并打印跳过提示，不引入任何额外依赖。
...
LLM 部分被明确标注为顾问性内容，绝不取代确定性输出；当 key 缺失或请求失败时审计会优雅降级（并打印提示），同一条命令在 CI 中依然安全。
...
src/agentic_testops/
  ...
  llm.py          可选的顾问性 LLM 分析（Anthropic API，标准库 HTTP）
