...
def render_markdown(report: AuditReport) -> str:
    ...
    if report.llm_explanations:
        model = report.llm_explanations[0].model
        lines.extend(
            [
                "## LLM Analysis",
                "",
                f"Advisory analysis generated by `{model}` from the structured failure evidence above. "
                "Verify against the deterministic diagnosis before acting.",
                "",
            ]
        )
        for index, explanation in enumerate(report.llm_explanations, start=1):
            lines.extend(
                [
                    f"### {index}. `{explanation.failure_nodeid}`",
                    "",
